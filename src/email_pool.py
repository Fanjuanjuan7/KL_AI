...
class EmailPool:
            ...
            for item in self.emails:
                if item["email"] == email:
                    if item.get("status") != status or (
                        reason and item.get("failure_reason") != reason
                    ):
                        item["status"] = status
                        if reason:
    ...
    def get_stats(self, mode_filter: Optional[str] = None) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            if mode_filter == "心蓝模式":
                ...
                target_emails = [
                    e for e in self.emails if e.get("auth_code", "").startswith("http")
                ]
                ...
                target_emails = [
                    ...
                    if not e.get("auth_code", "").startswith("http")
                ]
            else:
            ...
            used = sum(
                ...
                if e.get("status", EmailStatus.NEW) in completed_statuses
            )
            # 问题邮箱
            problem = sum(
                1
                for e in target_emails
                if e.get("status", EmailStatus.NEW) == EmailStatus.PROBLEM
            )

        return {"total_emails": total, "used_emails": used, "problem_emails": problem}
    ...
    def batch_delete_emails(self, emails: List[str]) -> Dict[str, any]:
        ...
        """
        result = {"success": True, "deleted_count": 0, "failed": []}
    ...
    def update_status(self, email: str, status: str, reason: str = "") -> bool:
        ...
        with self._lock:
            for e in self.emails:
                if e["email"] == email:
                    if e.get("status") != status or (
                        reason and e.get("failure_reason") != reason
                    ):
                        e["status"] = status
                        if reason:
    ...
    def _parse_email_line(self, line: str) -> Optional[Dict[str, str]]:
        ...
        # 2. 核心防御：扒掉 Excel/CSV 自动套上的单双引号，并二次去除隐形空格/换行
        parts = [p.strip().strip('"').strip("'").strip() for p in parts if p.strip()]
        ...
        # 3. 动态列数解析，彻底杜绝数据截断
        if len(parts) >= 4:
            # 命中 Outlook 专属 OAuth2 格式：邮箱 | 密码 | ClientID | RefreshToken
            password = parts[1]
            # 强行将第 3 段和第 4 段合并，并用 ||| 作为缝合线
            auth_code = parts[2] + "|||" + parts[3]
        elif len(parts) == 3:
            # 命中标准 3 段格式
            password = parts[1]
            auth_code = parts[2]
