...
CODE_REGEX_PATTERNS: List[str] = [
    r'验证码[\s:：]*([\d]{4,6})',  # 匹配"验证码：123456"
    r'verification code[\s:：]*([\d]{4,6})',  # 英文
    r'code[\s:：]*([\d]{4,6})',
    r'\b(\d{4,6})\b',  # 通用4-6位数字
]
...
class MailExtractor:
    ...
    def _log(self, message: str, level: str = "info") -> None:
        ...
        if self.custom_logger:
            ...
            prefix = (
                ...
                f"[Email:{self.email_account[:2]}***{self.email_account.split('@')[-1]}]"
            )
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            self.is_connected = False
            self.mail = None
            raise ConnectionError(f"Failed to connect to {self.imap_server}: {e}") from e
    ...
    def get_latest_verification_code(self) -> Optional[str]:
        ...
        try:
            ...
            for i, eid in enumerate(recent_ids):
                ...
                self._log(f"Checking email {i + 1}/{len(recent_ids)} (ID: {email_id_str})")
                ...
                if typ != 'OK':
                    self._log(f"Failed to fetch email {email_id_str}", level="warning")
                    continue
    ...
    def _parse_and_extract(self, msg_data: List[Tuple[bytes, bytes]]) -> Optional[str]:
        ...
        先尝试从纯文本中提取，如失败则尝试从 HTML 中提取。
        
        Args:
            msg_data: IMAP fetch 返回的邮件数据
        ...
        for response_part in msg_data:
            ...
            try:
                ...
                self._log(f"Failed to parse email message: {e}", level="warning")
                continue
    ...
    def _get_email_body(self, msg: Message) -> Tuple[str, str]:
        ...
        if msg.is_multipart():
            ...
            try:
                ...
                self._log(
                    f"Failed to decode body: {type(e).__name__}: {e}",
                    level="warning"
                )
